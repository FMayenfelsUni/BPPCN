...
public class RoutePayment extends Metric{
	...
	//protected double reputationDecrement = 1; //added line
...
/*			if (!dealt.contains(node)) {
	...
	public void computeData2(Graph g, Network n, HashMap<String, Metric> m) {
		...
		boolean[] excluded = new boolean[nodes.length]; //moved to this position so that excluded is not longer reset
		...
		for (int i = 0; i < endTime; i++) {
			...
			for (int ii = 0; ii < this.timeQueue[i].size(); ii++) {
				...
				//remove all nodes from excluded that finish their timeout

				List<Node> nodesToRemove = reputationManager.checkExclusionTimeout();
				for (Node node : nodesToRemove) {
					int nodeId = node.getIndex(); 
					excluded[nodeId] = false;
				}

				/*//check for reputationbased excluded nodes

				ArrayList<Node> excludedNodesToRemove = new ArrayList<>();
				for (Node excludedNode : ReputationManager.getExcludedNodes()) {
					// Check if the exclusion time is up and remove the node from the ReputationManager if necessary
					if (System.currentTimeMillis() >= excludedNode.getExclusionEndTime()) {
					  excludedNodesToRemove.add(excludedNode);
					} else {
						// Add the node to the excluded list in the RoutePayment class
						excluded.add(excludedNode);
					}
				}

				// Remove the nodes from the ReputationManager's excluded nodes HashMap
				for (Node nodeToRemove : excludedNodesToRemove) {
					ReputationManager.removeExcludedNode(nodeToRemove);
				}*/
				...
				if (!pps.isEmpty() && hopCount[id] < maxHopCount[id]) {
					...
					for (int j = 0; j < pps.size(); j++) {
						...
						double tempSum = 0.0;
						//reset excluded for future use (only the used nodes not the timeoutes ones)
						for (int l = 0; l < past.size(); l++) {
						...
						if (partVals != null) {
							...
							if(oldVals[id]<vals[id])
							{
								...
								//reduce reputation of current node 

								reputationManager.reduceReputationAndExclude(nodes[cur]);
								excluded[cur] = true;
							...
							//reputationManager.reduceReputationAndExclude(nodes[cur]);
							excluded[cur] = true;
							nodes[cur].setReputation(nodes[cur].getReputation() - reputationDecrement);
